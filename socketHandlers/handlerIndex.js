...
const message = async (payload, socket, recentMessages) => {
  if (payload.text !== '') {
    try {
      ...
      let createdMessage = await axios.post(`${BACKEND_URL}/api/v1/messages`, {
        text: cleanWords2,
        room: payload.room,
        username: payload.username,
      });
...
const createUser = async (payload, socket) => {
  try {
    let createdUser = await axios.post(`${BACKEND_URL}/api/v1/users`, {
      username: payload.username,
...
const getUsers = async (payload, socket) => {
  try {
    let getAllUsers = await axios.get(`${BACKEND_URL}/api/v1/users`);
